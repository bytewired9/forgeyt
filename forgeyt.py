...
def download(yturl, ftype, app_queue, stop_event, self):
    ...
    if audio:
        ...
        ydl_opts[
            "format"
        ] = f"bestvideo[ext={fileext}]+bestaudio[ext={fileext}]/best[ext={fileext}]"

    with YoutubeDL(ydl_opts) as ydl:
        try:
            ydl.download([yturl])
            app_queue.put(
                ("update_console", "ForgeYT made by ForgedCore8, 2023, SquawkSquad")
            )
...
class CustomMessageBox:
    ...
    def __init__(self, root, title, message):
        ...
        self.button = CTkButton(
            ...
            fg_color=app.buttoncolor,
            hover_color=app.buttoncolor_hover,
            width=80,
        )
...
class App(CTkFrame):
    ...
    def start_downloading(self):
        ...
        self.download_thread = Thread(
            target=download, args=(url, filetype, self.queue, self.stop_event, self)
        )
