...
import requests  # Required for sending the POST request.
import datetime  # Required for generating the timestamp.
...
currentversion = '2.0'
__dirname = str(pathlib.Path().resolve())
prompt = input
config_folder = os.path.join(appdata_path, 'ForgeYT')
...
def download(yturl, ftype, app_queue, stop_event, self): 
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            subprocess.run(['explorer', convert_to_absolute(download_path)])
...
def restartprompts():
    # doPrompts() -> This seems like a placeholder for now. Add the logic for this function.
    pass
...
#===========  Request Prompts  ============
#def doPrompts():
#    yturl = input("\033[92mEnter Youtube Link: \033[0m")
#
#    if yturl == 'issue':
#        problem = input("\033[1mWhat is the issue? \033[0m")
#        params = {
#            'username': "FORGEYT",
#            'content': " ",
#            'embeds': [
...
#            ]
#        }
#        response = requests.post('http://542355325.squawksquad.net:4200/', json=params)
#
#        if response.status_code == 200:
#            app_queue.put(("update_console", f'Your Issue report: "{problem}" has been posted!'))
#        else:
#            app_queue.put(("update_console", 'Error in posting your issue report.'))
#
#        return doPrompts()
#    else:
#        filetype = input("\033[96mWhat File Type? (Leave Blank For MP3): \033[0m")
#
#        if yturl == '':
#            app_queue.put(("update_console", "\033[41m\033[30mYoutube URL is Required.\033[0m"))
#            return doPrompts()
#        elif filetype not in filetypes:
#            app_queue.put(("update_console", f"\033[41m\033[30mFile Type Not Supported. Supported File Types are: {', '.join(filetypes)}\033[0m"))
#            return doPrompts()
#        else:
#            return download(yturl, filetype)
...
class CustomMessageBox:
    ...
    def __init__(self, root, title, message):
        ...
        self.label = ctk.CTkLabel(self.top, text=message, wraplength=250)
        self.label.pack(padx=10, pady=10)

        self.button = ctk.CTkButton(self.top, text="OK", command=self.destroy_messagebox, fg_color=app.buttoncolor, hover_color=app.buttoncolor_hover, width=80)
...
class App(ctk.CTkFrame):
    ...
    def __init__(self, root):
        ...
        self.left_frame = ctk.CTkFrame(self.root, fg_color=self.background_color2)
        self.left_frame.grid(row=0, column=0, sticky='nsew')
        self.right_frame = ctk.CTkFrame(root, fg_color=self.background_color1)
        ...
        self.console_output = ctk.CTkTextbox(self.right_frame, wrap=ctk.WORD, padx=20, pady=20, state=ctk.DISABLED)
        self.console_output.grid_forget()
        self.cursor_position = '1.0'
        self.setup_ui()
        self.last_newline_position = ctk.END
        self.queue = queue.Queue()
        self.stop_threads = threading.Event()
        self.root.protocol("WM_DELETE_WINDOW", self.close_app)
        self.check_queue()
        self.loading_label = ctk.CTkLabel(self.right_frame, text="Processing...", text_color='white', bg_color='transparent')
        self.loading_label.grid(pady=40)
        self.loading_label.grid_forget()
        self.start_image = ctk.CTkImage(light_image=Image.open(resource_path("download.png")), dark_image=Image.open(resource_path("download_dark.png")), size=(40, 40))
        self.start_button = ctk.CTkButton(self.right_frame, image=self.start_image, text="", command=self.start_downloading, fg_color=self.buttoncolor, hover_color=self.buttoncolor_hover,height=50, width=120)
        ...
        self.dropdown_menu = ctk.CTkComboBox(self.right_frame, values=self.dropdown_options, corner_radius=6)
        self.profile_entry = ctk.CTkEntry(self.right_frame, width=30)
    ...
    def start_downloading(self):
        ...
        self.download_thread = threading.Thread(target=download, args=(url, filetype, self.queue, self.stop_event, self))
    ...
    def cleanup_after_thread(self):
        ...
        self.loading_label.grid_forget()
        self.profile_entry.delete(0, ctk.END)
    ...
    def setup_ui(self):
        button_height = 57
        button_padding_y = 5
        self.forgeyt_image = ctk.CTkImage(Image.open(resource_path("ForgeYT.png")), size=(120, 120))
        self.home_image = ctk.CTkImage(light_image=Image.open(resource_path("Home.png")), dark_image=Image.open(resource_path("Home_dark.png")), size=(button_height-10, button_height-10))
        self.settings_image = ctk.CTkImage(light_image=Image.open(resource_path("Settings.png")), dark_image=Image.open(resource_path("Settings_dark.png")), size=(button_height-10, button_height-10))
        self.about_image = ctk.CTkImage(light_image=Image.open(resource_path("About.png")), dark_image=Image.open(resource_path("About_dark.png")), size=(button_height-10, button_height-10))
        self.console_image = ctk.CTkImage(light_image=Image.open(resource_path("Console.png")), dark_image=Image.open(resource_path("Console_dark.png")), size=(button_height-10, button_height-10))
        self.image_label = ctk.CTkLabel(self.left_frame, image=self.forgeyt_image, text="")
        self.image_label.grid(row=0, column=0, sticky="ew", padx=10, pady=(10, 2))
        self.image_label_label = ctk.CTkLabel(self.left_frame, text="ForgeYT", font=(self.font_name, 30, "bold"), text_color=self.font_color, bg_color="transparent")
        self.image_label_label.grid(row=1, column=0, sticky="ew", padx=10, pady=(1, 10))
        self.home_button = ctk.CTkButton(self.left_frame, image=self.home_image, text="", command=self.show_home, fg_color=self.buttoncolor, hover_color=self.buttoncolor_hover, height=button_height)
        self.home_button.grid(row=2, column=0, sticky="ew", padx=10, pady=button_padding_y)
        self.settings_button = ctk.CTkButton(self.left_frame, image=self.settings_image, text="", command=self.show_settings, fg_color=self.buttoncolor, hover_color=self.buttoncolor_hover, height=button_height)
        self.settings_button.grid(row=3, column=0, sticky="ew", padx=10, pady=button_padding_y)
        self.about_button = ctk.CTkButton(self.left_frame, image=self.about_image, text="", command=self.show_about, fg_color=self.buttoncolor, hover_color=self.buttoncolor_hover, height=button_height)
        self.about_button.grid(row=4, column=0, sticky="ew", padx=10, pady=button_padding_y)
        self.console_button = ctk.CTkButton(self.left_frame, image=self.console_image, text="", command=self.show_console, fg_color=self.buttoncolor, hover_color=self.buttoncolor_hover, height=button_height)
    ...
    def show_home(self):
        ...
        # Title Label
        self.scalable_elements['title_label'] = ctk.CTkLabel(self.right_frame, text="ForgeYT", font=(self.font_name, 50, "bold"), text_color=self.font_color, bg_color="transparent")
        self.scalable_elements['title_label'].grid(row=0, column=0, pady=50, sticky='nsew')
        # Profile URL Input Label
        self.scalable_elements['url_label'] = ctk.CTkLabel(self.right_frame, text="Enter Video URL", font=(self.font_name, 20), text_color=self.font_color, bg_color="transparent")
        ...
        self.dropdown_label = ctk.CTkLabel(self.right_frame, text="Select File Type", font=(self.font_name, 20), text_color=self.font_color, bg_color="transparent")
    ...
    def show_settings(self):
        ...
        # Title Label
        label = ctk.CTkLabel(self.right_frame, text="Settings", font=("Arial", 32, "bold"), text_color=self.font_color, justify="center")
        label.grid(row=0, column=0, columnspan=3, pady=10, padx=10, sticky="nsew")

        # Download Path Entry
        download_label = ctk.CTkLabel(self.right_frame, text="Download Path:", font=("Arial", 15, "bold"), text_color=self.font_color,)
        download_label.grid(row=1, column=0, padx=(10, 3), pady=10, sticky="w")

        self.filepath_entry = ctk.CTkEntry(self.right_frame, width=200, height=40)
        self.filepath_entry.grid(row=1, column=1, padx=(3,10), pady=10, sticky="w")

        self.select_folder_button = ctk.CTkButton(self.right_frame, text="Select", 
        ...
        # Theme Selection
        theme_label = ctk.CTkLabel(self.right_frame, text="Theme:", font=("Arial", 18), text_color=self.font_color)
        theme_label.grid(row=2, column=0, padx=(10,3) , pady=10, sticky="nsew")

        self.theme_var = tk.StringVar(value=config_data["theme"])  # Create a StringVar to hold the theme value
        self.theme_selector = ctk.CTkSegmentedButton(self.right_frame, values=["system","light", "dark"], 
        ...
        save_button = ctk.CTkButton(self.right_frame, text="Save", 
        ...
        reset_button = ctk.CTkButton(self.right_frame, text="Reset to Default", 
    ...
    def show_about(self):
        ...
        self.scrollable_frame = ctk.CTkScrollableFrame(self.right_frame, corner_radius=0, fg_color="transparent")
        self.scrollable_frame.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
        self.right_frame.grid_rowconfigure(0, weight=1)
        self.right_frame.grid_columnconfigure(0, weight=1)

         # Title
        self.about_title_label = ctk.CTkLabel(self.scrollable_frame, text="About ForgeYT", font=("Arial", 40, "bold"), text_color=self.font_color, bg_color="transparent")
        self.about_title_label.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")

        # Origin
        self.origin_label = ctk.CTkLabel(self.scrollable_frame, text="Origin:", font=("Arial", 18, "bold"), text_color=self.font_color, bg_color="transparent")
        self.origin_label.grid(row=1, column=0, padx=10, pady=2, sticky="nsew")
        self.origin_content = ctk.CTkLabel(self.scrollable_frame, wraplength=350, text=("ForgeYT was born out of the need for a reliable YouTube to MP3 converter. "
        ...
        # Initial Release
        self.release_label = ctk.CTkLabel(self.scrollable_frame, text="Initial Release:", font=("Arial", 18, "bold"), text_color=self.font_color, bg_color="transparent")
        self.release_label.grid(row=3, column=0, padx=10, pady=2, sticky="nsew")
        self.release_content = ctk.CTkLabel(self.scrollable_frame, wraplength=350, text=("The first iteration of ForgeYT was launched in February 2023. Beginning its journey as a Node.js CLI tool, it has undergone significant evolution to become the Python GUI application you see today."), text_color=self.font_color, bg_color="transparent")
        self.release_content.grid(row=4, column=0, padx=10, pady=2, sticky="nsew")

        # Challenges
        self.challenges_label = ctk.CTkLabel(self.scrollable_frame, text="Challenges:", font=("Arial", 18, "bold"), text_color=self.font_color, bg_color="transparent")
        self.challenges_label.grid(row=5, column=0, padx=10, pady=2, sticky="nsew")
        self.challenges_content = ctk.CTkLabel(self.scrollable_frame, wraplength=350, text=("Transitioning from Node.js to Python was no small feat. Integrating `yt-dlp` with Node.js's child-process presented unique challenges, but determination and innovation prevailed."), text_color=self.font_color, bg_color="transparent")
        self.challenges_content.grid(row=6, column=0, padx=10, pady=2, sticky="nsew")

        # Purpose & Usage
        self.purpose_label = ctk.CTkLabel(self.scrollable_frame, text="Purpose & Usage:", font=("Arial", 18, "bold"), text_color=self.font_color, bg_color="transparent")
        self.purpose_label.grid(row=7, column=0, padx=10, pady=2, sticky="nsew")
        self.purpose_content = ctk.CTkLabel(self.scrollable_frame, wraplength=350, text=("While ForgeYT was originally designed for use within a school environment, its versatility makes it apt for varied situations. Convert YouTube videos to audio with ease, regardless of where you are."), text_color=self.font_color, bg_color="transparent")
        self.purpose_content.grid(row=8, column=0, padx=10, pady=2, sticky="nsew")

        # acknowledgemnts
        self.ack_label = ctk.CTkLabel(self.scrollable_frame, text="Acknowledgments:", font=("Arial", 18, "bold"), text_color=self.font_color, bg_color="transparent")
        self.ack_label.grid(row=9, column=0, padx=10, pady=2, sticky="nsew")
        self.ack_content = ctk.CTkLabel(self.scrollable_frame, wraplength=350, text=("A heartfelt thank you to Mr. Z and DJ Stomp for their invaluable contributions. Additionally, a shout-out to the Python Discord community for their support."), text_color=self.font_color, bg_color="transparent")
        self.ack_content.grid(row=10, column=0, padx=10, pady=2, sticky="nsew")
        
        # Future
        self.ack_label = ctk.CTkLabel(self.scrollable_frame, text="Future:", font=("Arial", 18, "bold"), text_color=self.font_color, bg_color="transparent")
        self.ack_label.grid(row=11, column=0, padx=10, pady=2, sticky="nsew")
        self.ack_content = ctk.CTkLabel(self.scrollable_frame, wraplength=350, text=("ForgeYT stands proud at version 2.0. While this is likely its final form, the journey and impact it has had remain invaluable."), text_color=self.font_color, bg_color="transparent")
        self.ack_content.grid(row=12, column=0, padx=10, pady=2, sticky="nsew")

        self.version_label = ctk.CTkLabel(self.scrollable_frame, text=f"Current Version: {currentversion}",text_color=self.font_color, bg_color="transparent", font=("Arial", 10, "italic"))
    ...
    def insert_text_in_chunks(self, widget, text, chunk_size=50000):
        """
        Insert text into a Tkinter Text widget in chunks to avoid OverflowError.
        """
        for i in range(0, len(text), chunk_size):
            chunk = text[i:i + chunk_size]
            widget.insert(ctk.END, chunk)
    ...
    def write(self, s, from_queue=False):
        ...
        if self.console_output:
            self.console_output.configure(state=ctk.NORMAL)  # Enable widget for editing
            # Check for the '\r' character to overwrite the current line
            if '\r' in s:
                # Move the cursor to the start of the current line
                self.cursor_position = self.console_output.index(ctk.END + "-1 lines linestart")
                # Delete the entire current line
                self.console_output.delete(self.cursor_position, ctk.END + "-1 lines lineend")
                ...
                self.console_output.insert(ctk.END, s)
            # Update the cursor position
            self.cursor_position = self.console_output.index(ctk.END)
            # Make sure the view is always showing the most recent data
            self.console_output.yview(ctk.END)
            self.console_output.configure(state=ctk.DISABLED)  # Disable widget after editing
    ...
    def display_buffer(self):
        ...
        self.console_output.configure(state=ctk.NORMAL)
        self.console_output.delete(1.0, ctk.END)
        self.console_output.insert(ctk.END, final_output)
        self.console_output.configure(state=ctk.DISABLED)
...
root = ctk.CTk(fg_color=('#bef8ec', '#031b16'))
app = App(root)
root.mainloop()